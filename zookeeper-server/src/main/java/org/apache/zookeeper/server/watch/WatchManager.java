 ...
 * 服务器端的WatchManager，负责watch事件的触发和移除那些已经被触发的watch
 * WatchManager是一个统称：
 *      有两个WatchManager：
 *              一个是DataWatchManager  用来监听该数据节点变化
 *              一个是ChildWatchManager 用来监听子节点变化
 *
 */
public class WatchManager implements IWatchManager {
    private static final Logger LOG = LoggerFactory.getLogger(WatchManager.class);

    //内部存储结构  注册watch是会将ServerCnxn放到set里面，是从数据节点的维度来管理watch
    private final Map<String, Set<Watcher>> watchTable =
        new HashMap<String, Set<Watcher>>();
    //对应关系  会将ServerCnxn和节点形成对应关系，是从Watch的粒度来控制需要出发的数据节点
