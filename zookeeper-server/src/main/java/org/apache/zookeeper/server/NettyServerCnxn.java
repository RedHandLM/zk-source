...
public class NettyServerCnxn extends ServerCnxn {
    ...
    /**
     * 因为上一步存储了ServerCXN作为Watcher，所以直接调用process方法
     * @param event
     */
    @Override
    public void process(WatchedEvent event) {
        ...
        // Convert WatchedEvent to a type that can be sent over the wire  包装为WatcherEvent方便网络传输
        WatcherEvent e = event.getWrapper();

        try {
            //向客户端发送通知  由客户端的SendThread接收请求并处理watcher的回调逻辑
