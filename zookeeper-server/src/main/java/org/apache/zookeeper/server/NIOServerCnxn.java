...
public class NIOServerCnxn extends ServerCnxn {
    ...
    /*
     * Send close connection packet to the client, doIO will eventually close the underlying machinery
     * (like socket, selectorkey, etc...)
     */
    ...
    void handleWrite(SelectionKey k) throws IOException, CloseRequestException {
        ...
        /*
         * This is going to reset the buffer position to 0 and the limit to the size of the buffer, so that
         * we can fill it with data from the non-direct buffers that we need to send.
         */
        ...
        if (directBuffer == null) {
            ...
            for (ByteBuffer b : outgoingBuffers) {
                if (directBuffer.remaining() < b.remaining()) {
                    /*
                     * When we call put later, if the directBuffer is to small to hold everything, nothing will be
                     * copied, so we've got to slice the buffer if it's too big.
                     */
                    b = (ByteBuffer) b.slice().limit(directBuffer.remaining());
                }
                /*
                 * put() is going to modify the positions of both buffers, put we don't want to change the position
                 * of the source buffers (we'll do that after the send, if needed), so we save and reset the
    ...
    void doIO(SelectionKey k) throws InterruptedException {
        try {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                        "Exception causing close of session 0x" + Long.toHexString(sessionId) + ": " + e.getMessage());
            }
            close();
        } catch (IOException e) {
    ...
    private void closeSock() {
        ...
        LOG.debug("Closed socket connection for client " + sock.socket().getRemoteSocketAddress()
                + (sessionId != 0
    ...
    public static void closeSock(SocketChannel sock) {
        ...
        try {
            /*
             * The following sequence of code is stupid! You would think that only sock.close() is needed, but
             * alas, it doesn't work that way. If you just do sock.close() there are cases where the socket
