...
public class PrepRequestProcessor extends ZooKeeperCriticalThread implements RequestProcessor {
    ...
    private Map<String, ChangeRecord> getPendingChanges(MultiTransactionRecord multiRequest) {
        ...
        for (Op op : multiRequest) {
            ...
            /*
             * ZOOKEEPER-1624 - We need to store for parent's ChangeRecord of the parent node of a request. So
             * that if this is a sequential node creation request, rollbackPendingChanges() can restore previous
             * parent's ChangeRecord correctly.
             *
             * Otherwise, sequential node name generation will be incorrect for a subsequent request.
             */
     ...
     */
    static void checkACL(ZooKeeperServer zks, ServerCnxn cnxn, List<ACL> acl, int perm, List<Id> ids, String path,
            List<ACL> setAcls) throws KeeperException.NoAuthException {
        if (skipACL) {
            return;
        }
                    ...
                    for (Id authId : ids) {
                        if (authId.getScheme().equals(id.getScheme())
                                && ap.matches(new ServerAuthenticationProvider.ServerObjs(zks, cnxn),
                                        new ServerAuthenticationProvider.MatchValues(path, authId.getId(), id.getId(),
                                                perm, setAcls))) {
                            return;
                        }
                    }
    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record, boolean deserialize)
        ...
        switch (type) {
            ...
            case OpCode.delete:
                ...
                if (deserialize) ByteBufferInputStream.byteBuffer2Record(request.request, deleteRequest);
            ...
            case OpCode.setData:
                ...
                if (deserialize) ByteBufferInputStream.byteBuffer2Record(request.request, setDataRequest);
            ...
            case OpCode.reconfig:
                ...
                if (newMembers != null) { // non-incremental membership change
                    ...
                    try {
                        ...
                        request.qv =
                                QuorumPeerConfig.parseDynamicConfig(props, lzks.self.getElectionType(), true, false);
            ...
            case OpCode.setACL:
                ...
                if (deserialize) ByteBufferInputStream.byteBuffer2Record(request.request, setAclRequest);
            ...
            case OpCode.check:
                ...
                if (deserialize) ByteBufferInputStream.byteBuffer2Record(request.request, checkVersionRequest);
    ...
    private void validatePath(String path, long sessionId) throws BadArgumentsException {
        try {
            ...
            LOG.info("Invalid path {} with session 0x{}, reason: {}", path, Long.toHexString(sessionId),
                    ie.getMessage());
    ...
    protected void pRequest(Request request) throws RequestProcessorException {
        ...
        try {
            switch (request.type) {
                ...
                case OpCode.multi:
                    ...
                    for (Op op : multiRequest) {
                        ...
                        /*
                         * If we've already failed one of the ops, don't bother trying the rest as we know it's going to
                         * fail and it would be confusing in the logfiles.
                         */
                        if (ke != null) {
                            type = OpCode.error;
            ...
            if (e.code().intValue() > Code.APIERROR.intValue()) {
                LOG.info("Got user-level KeeperException when processing {} Error Path:{} Error:{}", request.toString(),
    ...
    private List<ACL> removeDuplicates(List<ACL> acl) {
        ...
        List<ACL> retval = new LinkedList<>();
        if (acl != null) {
            for (ACL a : acl) {
